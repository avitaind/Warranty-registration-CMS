...
class SellerController extends Controller
{
    ...
    public function InSalesSave(Request $request)
    {
        ...
        // $serialNum = \App\Models\product_number::where('product_number', $request->productNumber)->first();
        // $result = '';
        // if (isset($serialNum)) {
        //     $serialnoarr[] = explode(',', $serialNum->serial_number);
        ...
        try {
            ...
            // $checkproductnoexist = \App\Models\product_number::where('product_number', $request->productNumber)->pluck('id')->first();
            $checkproductnoexist = \App\Models\product_number::where('product_number', $request->productNumber)->pluck('id')->first();
            // dd($checkproductnoexist);

            if ($checkproductnoexist > 0) {
                // $checkserialnoexist = \App\Models\Sales::where('productNumber', $checkproductnoexist)->whereRaw("FIND_IN_SET('" . $request->serialNumber . "','serialNumber')")->first();
                $checkserialnoexist = \App\Models\Sales::where('user_id', Auth::user()->id)->whereRaw("NOT FIND_IN_SET('" . $request->serialNumber . "','serialNumber')")->first();
                // dd($checkserialnoexist);
                $serialnumarr[] = explode(',', $checkserialnoexist->serialNumber);
                $result = in_array($request->serialNumber, $serialnumarr[0]);
                // dd($result);

                if (in_array($request->serialNumber, $serialnumarr[0]) == false) {
                    $form = \App\Models\Sales::create([
                        'user_id'                => $request->user_id,
                        'productNumber'          => $request->productNumber,
                        'serialNumber'           => $request->serialNumber,
                        'productConfiguration'   => $request->productConfiguration,
                        'color'                  => $request->color,
                        'screenSize'             => $request->screenSize,
                        'saleDate'               => $request->saleDate,
                        'purchaseInvoice'        => $picture,
                    ]);
                    $form->save();

                    return Redirect::back()->with('msg', 'Product Configuration & Serial number');
                } else {
